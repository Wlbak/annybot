...
async def getbeans(ck, UA, client):
    ...
    try:
        ...
        while _7day:
            ...
            resp = await client.post(url, headers=headers, data=data, timeout=50)
            resp = resp.text
            res = json.loads(resp)
            if res['code'] == "0":
                if (len(res['detailList'])) != 0:
                    for i in res['detailList']:
                        for date in _7days:
                            if str(date) in i['date'] and int(i['amount']) > 0 and not re.search('退还|扣赠', i['eventMassage']):
                                ...
                                break
                        if i['date'].split(' ')[0] not in str(_7days):
                            _7day = False
                else:
                    _7day = False
            else:
